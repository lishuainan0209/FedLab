...
class Coordinator(object):
    ...
    def __init__(self, setup_dict: dict, mode: str = 'GLOBAL') -> None:
        self.rank_client_num_map = setup_dict
        self.mode = mode

    def map_id(self, id):
        """a map function from client id to (rank,local/global id)
            不同rank启动的顺序会影响到 __init__ 中 setup_dict 的顺序, 所以会影响
        Args:
            id (int): client id

        Returns:
            rank, id : rank in distributed group and local/global id. the id is >= 0
        """
        m_id = id
    ...
    def __str__(self) -> str:
        return "Coordinator map information: {} \nMap mode: {} \nTotal: {}".format(
            self.rank_client_num_map, self.mode, self.total)
