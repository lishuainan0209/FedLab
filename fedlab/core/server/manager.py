...
class ServerManager(NetworkManager):
    ...
    def __init__(self, network: DistNetwork, handler: ServerHandler, mode: str = "GLOBAL"):
    ...
    def setup(self):
        ...
        self.coordinator = Coordinator(rank_client_num_map, self.mode)
        if self._handler is not None:
            self._handler.total_clients = self.coordinator.total
...
class SynchronousServerManager(ServerManager):
    ...
    def __init__(self, network: DistNetwork, handler: ServerHandler, mode: str = "GLOBAL", logger: Logger = None):
        ...
        while self._handler.if_stop is not True:
            ...
            while True:
                ...
                if message_code == MessageCode.ParameterUpdate:
                    if self._handler.aggregation_process(payload):
                        break
                else:
    ...
    def activate_clients(self):
        ...
        # 选取参与下一轮循环的客户端编号,并找出对应的rank
        clients_this_round = self._handler.sample_clients()  # clients_this_round 是参与聚合的客户端的全局id
    ...
    def shutdown_clients(self):
        ...
        """
        client_list = range(self._handler.total_clients)
...
class AsynchronousServerManager(ServerManager):
    ...
    def aggregate_thread(self):
        ...
        while self._handler.if_stop is not True:
            ...
            self._handler.aggregation_process(payload)
