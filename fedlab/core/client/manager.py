...
class PassiveClientManager(ClientManager):
    ...
    def __init__(self, network: DistNetwork, trainer: ModelMaintainer, logger: Logger = None):
        ...
        while True:
            ...
            if message_code == MessageCode.Exit:
                ...
                if self._trainer.type == SERIAL_TRAINER:
                    self._trainer.train_process(payload=payload, id_list=id_list)

                elif self._trainer.type == ORDINARY_TRAINER:
                    assert len(id_list) == 1
                    self._trainer.train_process(payload=payload, id=id_list[0])
...
class ActiveClientManager(ClientManager):
    ...
    def __init__(self, network: DistNetwork, trainer: ClientTrainer, logger: Logger = None):
        super().__init__(network, trainer)
        self._LOGGER = Logger() if logger is None else logger
    ...
    def main_loop(self):
        ...
        while True:
            ...
            if message_code == MessageCode.Exit:
                ...
                # check the trainer type
                if self._trainer.type == SERIAL_TRAINER:
                    # warn 为什么直接rank - 1? 混合使用单客户端和serial客户端时会出问题吧?
                    self._trainer.train_process(id_list=[self._network.rank - 1], payload=payload)

                elif self._trainer.type == ORDINARY_TRAINER:
                    self._trainer.train_process(id=self._network.rank - 1, payload=payload)

                self.synchronize()

            else:
                raise ValueError("Invalid MessageCode {}. Please check MessageCode Enum.".format(message_code))
